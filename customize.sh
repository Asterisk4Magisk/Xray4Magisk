...
installModule() {
  ...
  [ -f ${config_file} ] ||
    unzip -j -o "${ZIPFILE}" "xray/etc/confs/${config}" -d ${module_path}/confs >&2
  ...
  echo "versionCode=20220925" >>$MODPATH/module.prop
  echo "# Settings" >${module_path}/xray.config
  echo "config=$config" >>${module_path}/xray.config
